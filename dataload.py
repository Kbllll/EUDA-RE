...
def load_Yale32():
    cache_file = os.path.join(CACHE_DIR, "yale32.npz")
    if os.path.exists(cache_file):
        print("✅ 从缓存加载 Yale32")
        data = np.load(cache_file)
        return data["X"], data["y"], data["img_shape"], data["num_classes"]

    base_dir = os.path.join(DATASETS_DIR, "yalefaces")
    if not os.path.exists(base_dir):
        raise FileNotFoundError(
            f"❌ 未找到 Yale 数据集！请解压到: {base_dir}\n"
            "文件应命名为: subject01.centerlight, subject01.sad, ..."
        )

    images, labels = [], []
    # 获取所有以 'subject' 开头的文件（无论有无扩展名）
    files = [f for f in os.listdir(base_dir) if f.startswith('subject')]
    files.sort()

    for file in files:
        try:
            # 从文件名提取标签：subject01.sad → label = 0
            parts = file.split('.')
            if len(parts) < 2:
                continue  # 无效文件
            label_str = parts[0][7:]  # 'subject01' → '01'
    ...
    if len(images) == 0:
        raise RuntimeError("未加载任何 Yale 图像！请检查 datasets/yalefaces/ 文件名。")
...
def load_dataset(name):
    ...
    if name == "Digits":
        ...
        raise ValueError("不支持的数据集。支持: 'Digits', 'ORL32', 'Yale32'")
